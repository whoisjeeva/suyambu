...
suspend fun Piper.executeTagNameOf(statement: JSONObject, onError: (e: Exception) -> Unit): Any? {
    try {
        Log.d("hello", "execute_tag_name_of")
        ...
        for (el in els) {
            ...
            onUiThread {
                ...
                """.trimIndent()) {
                    val tagName = if (it == "null") null else it.lowercase()
